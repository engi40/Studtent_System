  ...
  function parseDate(dateString) {
    ...
    return new Date(`20${year}`, month - 1, day); // Assuming the year is in the 21st century
        ...
        const handleSearch5 = (e) => {
            const query5 = e.target.value;
            
            setSearchQuery5(query5);
          
            const filtered = view.filter(item => {
              // Customize the conditions based on your filtering requirements
              return (
                item.firstName.toLowerCase().includes(query5.toLowerCase()) ||
                item.state.toLowerCase().includes(query5.toLowerCase()) ||
                item.ID.toLowerCase().includes(query5.toLowerCase()) ||
                item.pass.toLowerCase().includes(query5.toLowerCase()) ||
                item.email.toLowerCase().includes(query5.toLowerCase()) ||
                item.gender.toLowerCase().includes(query5.toLowerCase()) ||
                item.grade.toLowerCase().includes(query5.toLowerCase()) ||
                item.major.toLowerCase().includes(query5.toLowerCase()) ||
                item.date1.toLowerCase().includes(query5.toLowerCase()) ||
                item.Type1.toLowerCase().includes(query5.toLowerCase()) ||
                item.type.toLowerCase().includes(query5.toLowerCase())||
                item.discount.toLowerCase().includes(query5.toLowerCase()) ||
                item.pay.toLowerCase().includes(query5.toLowerCase())||
                item.pay1.toLowerCase().includes(query5.toLowerCase())||
                item.date2.toLowerCase().includes(query5.toLowerCase())||
                item.service1.toLowerCase().includes(query5.toLowerCase())||
                item.service2.toLowerCase().includes(query5.toLowerCase())||
                item.service3.toLowerCase().includes(query5.toLowerCase())||
                item.dateSer1.toLowerCase().includes(query5.toLowerCase())||
                item.dateSer2.toLowerCase().includes(query5.toLowerCase())||
                item.dateSer3.toLowerCase().includes(query5.toLowerCase())||
                item.pay2.toLowerCase().includes(query5.toLowerCase())||
                item.pay3.toLowerCase().includes(query5.toLowerCase())||
                item.option.toLowerCase().includes(query5.toLowerCase())||
                item.address.toLowerCase().includes(query5.toLowerCase())||
                item.telephone.toLowerCase().includes(query5.toLowerCase())||
                item.book.toLowerCase().includes(query5.toLowerCase())||
                item.dateBook.toLowerCase().includes(query5.toLowerCase())||
                item.notes.toLowerCase().includes(query5.toLowerCase())||
                item.debt.toLowerCase().includes(query5.toLowerCase())||
                item.debts.toLowerCase().includes(query5.toLowerCase())||
                item.totaldebt.toLowerCase().includes(query5.toLowerCase())||
                item.sertotal.toLowerCase().includes(query5.toLowerCase())||
                item.execuse.includes(query5.toLowerCase())||
                item.payyys.includes(query5.toLowerCase())
              );
            });
    ...
    {employeeStates.map(state => (
      <option key={state} value={state}>{state}</option>
                ...
                />
                        ...
                        <th className='cell' style={{textAlign:"center" , width: '5%'}}>البريد الإلكترونى 
                        <input className='contain-table' style={{textAlign:"center" , width: '100px'}}
                  type="text"
                  placeholder="بحث"
                  value={searchQuery3}
                ...
                />
                        ...
                        <th className='cell' style={{textAlign:"center" , width: '3%'}}>النوع
                        <select
                        style={{
                          width: '90px',
                              padding: '8px',
...
>
  ...
  {employeeGrades.map(grade => (
    <option key={grade} value={grade}>{grade}</option>
                        ...
                        <th className='cell' style={{textAlign:"center" , width: '5%'}}>التخصص - القسم
                        <select
                       style={{
                        width: '100px',
                            padding: '8px',
...
>
  ...
  {employeeOptions.map(option => (
    <option key={option} value={option}>{option}</option>
                        ...
                        <th className='cell' style={{textAlign:"center", width: '7%'}}>العنوان
                        <input className='contain-table' style={{textAlign:"center" , width: '100px'}}
                  type="text"
                  placeholder="بحث"
                  value={searchQuery5}
                                ...
                                filteredEmployees.map((employee, index) => (
                                    ...
                                    >
                                        تعديل
                                                    ...
                                                    <Link className='round-button' to={`/search/${employee.id}`}>
